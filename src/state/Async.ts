...
export const asyncError: (error: string) => AsyncError = (error: string) => ({ type: '__error__', error });
export const asyncLoaded: <T>(value: T) => AsyncLoaded<T> = <T>(value: T) => ({ type: '__loaded__', value });

export const isAsyncEmpty = <T>(async: Async<T>): async is AsyncEmpty => async.type === '__empty__';
export const isAsyncLoading = <T>(async: Async<T>): async is AsyncLoading => async.type === '__loading__';
export const isAsyncError = <T>(async: Async<T>): async is AsyncError => async.type === '__error__';
